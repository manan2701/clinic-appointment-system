...
const JWT_SECRET =
  "a_very_strong_and_secret_jwt_key_for_careysite_project_2025";
...
exports.registerClinic = async (req, res) => {
  ...
  if (!name || !email || !password || !licenseNumber)
    return res.status(400).json({ message: "All fields are required" });
  if (!validLicenses.has(licenseNumber))
  ...
  res.status(201).json({
    message: "Clinic registration request submitted. Awaiting admin approval.",
  });
};
  ...
  if (user.role === "Clinic" && user.isApproved !== "approved") {
    const message =
      user.isApproved === "pending"
        ? "Your clinic registration is still pending approval."
        : "Your clinic registration has been rejected.";
